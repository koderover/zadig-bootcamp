...
3. 着这里输入当前所使用的 ZadigX 培训环境的访问入口域名 ‘edu.koderover.com’
...
7. 在这里选择你个人GitHub 账号中的本课程培训代码仓库，如果是一个复杂的应用，有可能需要点击后方的加号按钮，添加其他的代码仓库，作为需要构建的源代码。
...
* 用 docker push 命令将镜像推送到项目所制定的镜像仓库
...
* 本课程中演示的自定义方式，这样的简单的代码片段，完全在 ZadigX 的图形界面中做管理，在项目初期，代码构建流程和工具多变，且需要临时调整的情况下。
...
2. 查看前度基于多阶段 Dockerfile 的构建流程日志
...
前端页面在浏览器中可以正常显示；但是，后端构建时间现实异常。我们发现了这个产品有明显的 bug ，我们要在后续的课程中修复这个 bug。
...
4. 填写描述
5. 选择代码库
6. 选择目标分支的 “这则表达式配置”，输入 `/*`；触发所有分支的提交
7. 选择自动化构建后的需要部署更新的环境
8. “动态的选择空闲的环境更新” ，这个选项会在多个可选的 k8s 环境中自动选择，最空闲的集群。
9. 选择需要部署的目标服务
10. 勾选所有选项
11. 点击“确认”按钮
...
1. 点击 unit-test 套件右侧的 加号➕ 按钮，将此产品的 qa 和 dev 工作流与之，进行强关联。
2. 点击 init-test 套件右侧的 加号➕ 按钮，将此产品的 qa 和 dev 工作流与之，进行强关联。
3. 点击 perf-test 套件右侧的 加号➕ 按钮，将此产品的 qa 和 ops 工作流与之，进行强关联。
...
2. 点击 “执行” 按钮，观察，此工作流执行后绑定的测试套件的执行。

![](images/2023-10-17_20-39-25.jpg)

1. 进入测试管理页面，点击 unit-test 测试套件右端的齿轮 ⚙ 按钮，进入，测试的高级配置选项。
2. 点击代码变更触发的“添加”按钮

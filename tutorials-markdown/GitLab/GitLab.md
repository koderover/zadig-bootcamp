...
这里，我们使用从代码库同步的方式：点击`从代码库同步`按钮 -> 选择仓库信息及文件目录 -> 点击`同步`按钮。Zadig 支持一次性导入多个服务，选择 `k8s-yaml` 文件目录可同步此次案例中所需的 `backend` 服务和 `frontend` 服务。
...
- 待工作流执行完毕，进入 `项目`->`voting`->`集成环境`，可看到服务的镜像已被自动触发的工作流更新。
